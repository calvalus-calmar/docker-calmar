...
# second link directories that we do not need to make persistent
cd /opt/tomcat/
...
# third create directories that we wish to allow persistence
cd /usr/local/tomcat/
...
# fourth, setup Calvalus portal
if [ ! -d "/opt/tomcat/webapps/calvalus-portal/" ]; then
  echo "Setting up Calvalus portal"
  mv /calvalus-portal /opt/tomcat/webapps
fi
